...
TEST_F(FixtureTestShellMock, TEST_SHELL_HELP_READLINES) {
	EXPECT_CALL(mock, help).WillOnce(Return(1));
	int ret = mock.help();
...
TEST_F(FixtureTestShellMock, DISABLED_TEST_SHELL_WRITE_INVLIAD_LBA) {
	EXPECT_THROW(mock.write(999, "0x123"), invalid_argument);
}

TEST_F(FixtureTestShellMock, DISABLED_TEST_SHELL_WRITE_INVLIAD_DATA) {
